    ...
    <>
      <Dialog open={open} onOpenChange={() => onClose()}>
        <DialogContent className="max-w-2xl bg-card border-border/50 text-foreground max-h-[90vh] overflow-y-auto">
          ...
          {/* Single image frame with Photo / ESASky toggle */}
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex rounded-md border border-border overflow-hidden text-[11px]">
                <button
                  onClick={() => setViewMode("photo")}
                  className={`px-3 py-1 transition-colors ${viewMode === "photo" ? "bg-primary/20 text-primary font-medium" : "text-muted-foreground hover:bg-secondary/50"}`}
                >
                  🖼️ Photo
                </button>
                <button
                  onClick={() => setViewMode("esasky")}
                  className={`px-3 py-1 transition-colors border-l border-border ${viewMode === "esasky" ? "bg-primary/20 text-primary font-medium" : "text-muted-foreground hover:bg-secondary/50"}`}
                >
                  🔭 ESASky
                </button>
              </div>
              {viewMode === "photo" && wikiImage?.pageUrl && (
                ...
                >
                  <ExternalLink className="w-3 h-3" /> {wikiImage.source === "wikipedia" ? "Wikipedia" : "NASA SkyView"}
                </a>
              )}
              ...
              {viewMode === "photo" ? (
                ...
                <div className="w-full h-72">
                  {obj.ra != null && obj.dec != null ? (
                    <img
                      key={obj.catalog_id}
                      src={getSkyImageUrl(obj.ra, obj.dec, obj.size_max, 600, 400) ?? ""}
