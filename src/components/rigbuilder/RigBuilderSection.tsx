...
interface RigBuilderSectionProps {
  rigPicks: RigPicks;
  onRigPicksChange: (picks: RigPicks) => void;
}
...
const RigBuilderSection = ({ rigPicks, onRigPicksChange }: RigBuilderSectionProps) => {
  ...
  // Rig pick handlers
  const pickTelescope = (id: string) => onRigPicksChange({ ...rigPicks, telescope: rigPicks.telescope === id ? null : id });
  const pickCamera = (id: string) => onRigPicksChange({ ...rigPicks, camera: rigPicks.camera === id ? null : id });
  const pickMount = (id: string) => onRigPicksChange({ ...rigPicks, mount: rigPicks.mount === id ? null : id });
  const toggleFilter = (id: string) => onRigPicksChange({
    ...rigPicks,
    filterIds: rigPicks.filterIds.includes(id) ? rigPicks.filterIds.filter(i => i !== id) : [...rigPicks.filterIds, id],
  });
  const toggleAccessory = (id: string) => onRigPicksChange({
    ...rigPicks,
    accessories: rigPicks.accessories.includes(id) ? rigPicks.accessories.filter(i => i !== id) : [...rigPicks.accessories, id],
  });
        ...
        <TabsContent value="telescopes">
          <EquipmentTab loading={loadingScopes} searchBar={searchBar} resultCount={filteredScopes.length} searchQuery={searchQuery}
            ...
            compareTable={compareIds.telescopes.length >= 2 ? (
              <CompareTable items={telescopes?.filter(t => compareIds.telescopes.includes(t.id)) ?? []} getImage={t => t.image_url}
                columns={[
        ...
        <TabsContent value="cameras">
          <EquipmentTab loading={loadingCams} searchBar={searchBar} resultCount={filteredCams.length} searchQuery={searchQuery}
            ...
            compareTable={compareIds.cameras.length >= 2 ? (
              <CompareTable items={cameras?.filter(c => compareIds.cameras.includes(c.id)) ?? []} getImage={c => c.image_url}
                columns={[
        ...
        <TabsContent value="mounts">
          <EquipmentTab loading={loadingMounts} searchBar={searchBar} resultCount={filteredMounts.length} searchQuery={searchQuery}
            ...
            compareTable={compareIds.mounts.length >= 2 ? (
              <CompareTable items={mounts?.filter(m => compareIds.mounts.includes(m.id)) ?? []} getImage={m => m.image_url}
                columns={[
        ...
        <TabsContent value="filters">
          <EquipmentTab loading={loadingFilters} searchBar={searchBar} resultCount={filteredFilts.length} searchQuery={searchQuery}
            ...
            compareTable={compareIds.filters.length >= 2 ? (
              <CompareTable items={filters?.filter(f => compareIds.filters.includes(f.id)) ?? []} getImage={f => f.image_url}
                columns={[
...
export default RigBuilderSection;
